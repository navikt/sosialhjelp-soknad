...
export interface Adresse {
	"adresse": null | string;
	"husnummer": any; // null | string;
	"husbokstav": any; // null | string;
...
class AdresseTypeahead extends React.Component<Props, StateProps> {
	...
	executeFetch(value: string) {
		...
		if (value === "sanner") {
			console.warn("bruker testdata på adressesøk");
			const testAdresser: Adresse[] = [ {
				"adresse": "SANNERGATA",
				"husnummer": null as null,
				"husbokstav": null as null,
				"kommunenummer": "1201",
				"kommunenavn": "Bergen",
				"postnummer": "1337",
				"poststed": "Leet",
			...
			this.setState({adresser: testAdresser});
			return;
		}
	...
	visIkon(antallAktiveSok: number) {
		return (
		...
		return (
			<div className="navAutcomplete" id="digisosTypeahead">
				<Autocomplete
					...
					selectOnBlur={false}
				/>
				{this.visIkon(this.state.antallAktiveSok)}
