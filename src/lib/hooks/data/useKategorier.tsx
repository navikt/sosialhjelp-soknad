...
interface Category {
    ...
    subCategories?: {key: DigisosLanguageKey | string; text: string}[];
    icons: any[];
    text: string;
}
...
interface SelectableCategory extends Category {
    selected: boolean;
    subCategories?: {key: DigisosLanguageKey | string; text: string; selected: boolean}[];
}
...
export const CATEGORIES: Category[] = [
    {key: "begrunnelse.kategorier.mat", icons: [<CoffeeIcon />], text: "Mat"},
    {
        ...
        subCategories: [
            ...
            {key: "Midlertidig bolig", text: "Midlertidig bolig"},
            {key: "Innbo og utstyr", text: "Innbo og utstyr"},
            {key: "Depositum", text: "Depositum"},
            {key: "Kommunale avgifter", text: "Kommunale avgifter"},
            {key: "Avdrag og renter på boliglån", text: "Avdrag og renter på boliglån"},
            {key: "Flytting", text: "Flytting"},
        ],
        icons: [<HouseIcon />],
        text: "Bolig",
    },
    ...
    {key: "begrunnelse.kategorier.tannlege", icons: [<FirstAidKitIcon />], text: "Tannlege"},
    {
        ...
        subCategories: [
            {key: "Har ikke penger til mat i dag", text: "Har ikke penger til mat i dag"},
            {key: "Har ikke et sted å bo i natt", text: "Har ikke et sted å bo i natt"},
            {
                key: "Strømmen er stengt / stenges i dag eller i morgen",
                text: "Strømmen er stengt / stenges i dag eller i morgen",
            },
            {
                key: "Har ikke penger til helsehjelp jeg må ha i dag",
                text: "Har ikke penger til helsehjelp jeg må ha i dag",
            },
            {
                key: "Blir kastet ut hjemmefra i dag eller i morgen",
                text: "Blir kastet ut hjemmefra i dag eller i morgen",
            },
            {key: "Har ikke penger til varme hjemme i dag", text: "Har ikke penger til varme hjemme i dag"},
            {key: "Har ikke penger til å komme meg hjem i dag", text: "Har ikke penger til å komme meg hjem i dag"},
        ],
    ...
    {key: "begrunnelse.kategorier.transport", icons: [<BusIcon />, <TrainIcon />, <TramIcon />], text: "Transport"},
    {key: "begrunnelse.kategorier.husholdning", icons: [<BucketMopIcon />], text: "Husholdningsartikler"},
...
const useKategorier = (
    ...
    const toggle = (category: string, subCategory?: string) => {
        ...
        return dispatch({type: "toggle", category});
    };
    ...
    const onSubmit = handleSubmit((begrunnelseFrontend: BegrunnelseFrontend) => {
        if (isKategorierEnabled) {
            ...
            put({hvaSokesOm: JSON.stringify(jsonString)});
        } else {
            put(begrunnelseFrontend);
        }
    }, inhibitNavigation);
