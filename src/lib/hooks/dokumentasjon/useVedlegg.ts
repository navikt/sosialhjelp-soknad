...
export const useVedlegg = (dokumentasjonType: DokumentasjonDtoType) => {
     ...
     */
    useEffect(() => dispatch({type: "initialize", data: dokumentasjon?.dokumentasjon}), [dokumentasjon?.dokumentasjon]);
    ...
    const uploadDocument = async (file: File) => {
        ...
        if (file.size > TEN_MEGABYTE_COMPAT_FALLBACK) {
            setError(t(REST_FEIL.FOR_STOR, "", {maxUploadSize: humanizeFilesize(TEN_MEGABYTE_COMPAT_FALLBACK)}));
