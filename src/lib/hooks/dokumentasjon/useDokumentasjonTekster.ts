...
import {opplysningSpec, permutations} from "../../opplysninger";
...
export const useDokumentasjonTekster = (opplysningType: DokumentasjonDtoType): DokumentasjonTexts => {
    const {componentDescription} = opplysningSpec[opplysningType];
    const {numRows} = permutations[componentDescription];
