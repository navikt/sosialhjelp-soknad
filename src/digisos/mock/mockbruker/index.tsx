...
class MockBruker extends React.Component<Props, OwnState> {
    ...
    settInnListeOverArbeidsforhold() {
        ...
        this.state.arbeidsforhold_liste.forEach((forhold: NyttArbeidsforholdObject, key: number) => {
            a.push(
                <div className="mock-thing" key={key}>
                    {this.renderForholdRad(forhold, key)}
                </div>
            );
        });
    ...
    handleLeggTilNySkattetatebUtbetaling(
        kalendermaaned: string,
        orgNummer: number,
        nyInntekt: NySkattetatenInntektObject,
        nyTrekk: NySkattetatenForskuddstrekkObject | undefined
    ) {
        const skattetatenManeder: NyPeriodeOgUtbetaler[] = this.state.periodeOgUtbetaler;
        let skattetatenManedList = skattetatenManeder.filter(
            (it) => it.kalendermaaned === kalendermaaned && it.opplysningspliktigId === orgNummer.toString()
        );
    ...
    handleLeggTilNyNavUtbetaling(nyNavUtbetaling: NyNavUtbetalingObject, nyNavYtelse: NyNavYtelseObject) {
        ...
        let treffListe = nyNavUtbetalingsListe.filter(
            (it) =>
    ...
    settInnListeOverUtbetalinger() {
        ...
        this.state.bostotteDto.utbetalinger.forEach((utbetaling: NyUtbetalingObject, key: number) => {
            a.push(
                <div className="mock-thing" key={key}>
                    {this.renderUtbetalingRad(utbetaling, key)}{" "}
                </div>
            );
        });
        ...
        return (
            <div className="mock-liste-of-things">
                ...
                {sak.status === "VEDTATT" && sak.vedtak && (
                    <div>
                        Vedtak: {sak.vedtak.type} - {sak.vedtak.beskrivelse}
                    </div>
                )}
                <button
                    onClick={() => {
    ...
    start() {
            ...
            if (this.state.arbeidsforhold_liste.length > 0) {
                this.state.arbeidsforhold_liste.forEach((forhold: NyttArbeidsforholdObject, key: number) => {
                    if (forhold.type === ArbeidsforholdType.PERSON) {
                        mocksystemdata.settArbeidsforholdArbeidsgiverPerson(
                            forhold.id,
                            forhold.startDato,
                            forhold.sluttDato,
                    ...
                    if (forhold.type === ArbeidsforholdType.ORGANISASJON) {
                        mocksystemdata.settArbeidsforholdArbeidsgiverOrganisasjon(
                            forhold.id,
                            forhold.startDato,
                            forhold.sluttDato,
                            forhold.stillingsProsent,
        ...
        if (this.state.barn) {
            this.state.barn_liste.forEach((barn: NyttBarnObject, key: number) => {
                if (barn.doedsdato) {
                    mocksystemdata.settBarnMedDoedsdato(
                        barn.ident,
                        barn.fornavn,
                        barn.mellomnavn,
                        barn.etternavn,
                        barn.doedsdato_value
                    );
                } else {
                    if (barn.sammeBostedsadresse) {
                        mocksystemdata.settBarnSammeBostedsadresse(
                            barn.ident,
                            barn.fornavn,
                            barn.mellomnavn,
                            barn.etternavn
                        ...
                        mocksystemdata.settBarnIkkeSammeBostedsadresse(
                            barn.ident,
                            barn.fornavn,
                            barn.mellomnavn,
                            barn.etternavn
        ...
        Promise.all([
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getTelefonPath(),
                JSON.stringify(mocksystemdata.getTelefonJson())
            ),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getFamiliePath(),
                JSON.stringify(mocksystemdata.getFamilieJson())
            ),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getBrukerprofilPath(),
                JSON.stringify(mocksystemdata.getBrukerprofilJson())
            ),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getOrganisasjonPath(),
                JSON.stringify(mocksystemdata.getOrganisasjonJson())
            ),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getArbeidPath(),
                JSON.stringify(mocksystemdata.getArbeidJson())
            ),
            fetchPost("internal/mock/tjeneste/" + mocksystemdata.getUtbetalingPath(), navUtbetalingerStreng),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getUtbetalingPath() + "_feiler",
                JSON.stringify(this.state.navUtbetalinger_feiler)
            ),
            fetchPost("internal/mock/tjeneste/" + mocksystemdata.getSkattetatenPath(), skattetatenStreng),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getSkattetatenPath() + "_feiler",
                JSON.stringify(this.state.skatt_feiler)
            ),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getBostottePath(),
                JSON.stringify(mocksystemdata.getBostotteJson())
            ),
            fetchPost(
                "internal/mock/tjeneste/" + mocksystemdata.getBostottePath() + "_feiler",
                JSON.stringify(this.state.bostotte_feiler)
            ),
        ]).then(() => {
            ...
            <div className="mock-body">
                <div className="mock-egendefinert-section">
                    ...
                    <MockDataBolkWrapper tittel="Personalia">
                        ...
                        >
                            <option value="NOR" key="NOR">
                                NOR
                            </option>
                            <option value="SWE" key="SWE">
                                SWE
                            </option>
                            <option value="DEN" key="DEN">
                                DEN
                            </option>
                            <option value="GER" key="GER">
                                GER
                            </option>
                    ...
                    <MockDataBolkWrapper
                        tittel="Midlertidig adresse (Matrikkel)"
                        value={this.state.midlertidig_matrikkeladresse}
                    ...
                    <MockDataBolkWrapper
                        tittel="Bankkontonummer"
                        value={this.state.bankkonto}
                    ...
                    <MockDataBolkWrapper
                        ...
                        value={this.state.arbeidsforhold}
                    ...
                    >
                        <Collapse className="mock-block-collapse" isOpened={this.state.arbeidsforhold}>
                            ...
                            <NyttArbeidsforhold
                                onLeggTilNyttArbeidsforhold={(nyttArbeidsForhold: NyttArbeidsforholdObject) =>
                                    this.handleLeggTilNyttArbeidsforhold(nyttArbeidsForhold)
                                }
                            />
                        </Collapse>
                    </MockDataBolkWrapper>
                    ...
                    >
                        <Collapse className="mock-block-collapse" isOpened={this.state.ektefelle}>
                            ...
                            <div className="mock-radiogroup">
                                <div className="mock-block-tittel">Har samme bostedsadresse:</div>
                                <div className="mock-radiogroup">
                                    <Radio
                                        onChange={() => this.setState({ektefelle_medSammeBostedsadresse: true})}
                                        label="Ja"
                                        name="samme_bostedsadresse"
                                        value={"ja"}
                                        defaultChecked={true}
                                    />
                                    <Radio
                                        onChange={() => this.setState({ektefelle_medSammeBostedsadresse: false})}
                                        label="Nei"
                                        name="samme_bostedsadresse"
                                        value={"nei"}
                                    />
                            ...
                            <div className="mock-radiogroup">
                                <div>Har kode 6 eller 7</div>
                                <div className="mock-radiogroup">
                                    <Radio
                                        onChange={() => this.setState({ektefelle_medKode: ""})}
                                        label="Nei"
                                    ...
                                    />
                                    <Radio
                                        onChange={() => this.setState({ektefelle_medKode: "6"})}
                                        label="6"
                                        name="diskresjonskode"
                                        value={"6"}
                                    />
                                    <Radio
                                        onChange={() => this.setState({ektefelle_medKode: "7"})}
                                        label="7"
                                        name="diskresjonskode"
                                        value={"7"}
                                    />
                    ...
                    <MockDataBolkWrapper
                        tittel="Skattetaten utbetalinger"
                        value={this.state.harSkattetatenUtbetalinger}
                    ...
                    >
                        <Collapse className="mock-block-collapse" isOpened={this.state.harSkattetatenUtbetalinger}>
                            ...
                            <NySkattetatenUtbetaling
                                onLeggTilNyUtbetaling={(
                                    maned: string,
                                    orgNummer: number,
                                    nyUtbetaling: NySkattetatenInntektObject,
                                    nyTrekk: NySkattetatenForskuddstrekkObject | undefined
                    ...
                    <MockDataBolkWrapper
                        tittel="Skal Skatteetaten feile?"
                        value={this.state.skatt_feiler}
                        callback={(value: boolean) => this.setState({skatt_feiler: value})}
                        booleanText_true="Ja!"
                        booleanText_false="Nei."
                    ></MockDataBolkWrapper>

                    <MockDataBolkWrapper
                        tittel="Bostøtte utbetalinger"
                        value={this.state.bostotteUtbetalinger}
                    ...
                    >
                        <Collapse className="mock-block-collapse" isOpened={this.state.bostotteUtbetalinger}>
                            ...
                            <NyBostotteUtbetaling
                                onLeggTilNyUtbetaling={(nyUtbetaling: NyUtbetalingObject) =>
                                    this.handleLeggTilNyUtbetaling(nyUtbetaling)
                                }
                            />
                        </Collapse>
                    </MockDataBolkWrapper>
                    ...
                    <MockDataBolkWrapper
                        tittel="Bostøtte saker"
                        value={this.state.bostotteSaker}
                    ...
                    <MockDataBolkWrapper
                        tittel="Utbetalinger fra NAV"
                        value={this.state.navUtbetaling}
                    ...
                    >
                        <Collapse className="mock-block-collapse" isOpened={this.state.navUtbetaling}>
                            ...
                            <NyNavUtbetaling
                                onLeggTilNyNavUtbetaling={(
                                    nyNavUtbetaling: NyNavUtbetalingObject,
                                    nyNavYtelse: NyNavYtelseObject
                    ...
                    <MockDataBolkWrapper
                        tittel="Skal utbetalinger fra NAV feile?"
