...
import Steg1 from "./steg1personopplysninger/SelvstendigNaringsdrivendeSteg1Personopplysninger";
import Steg8 from "./steg2okonomiskeopplysninger/SelvstendigNaringsdrivendeSteg2Okonomiskeopplysninger";
import Oppsummering from "./steg3oppsummering/SelvstendigNaringsdrivendeSteg3Oppsummering";
...
const SelvstendigNaringsdrivendeSkjemaRouter: React.FC<Props> = (props: Props) => {
    ...
    useEffect(() => {
        ...
        const match = matchPath(props.location.pathname, {
            path: "/selvstendignaringsdrivende/skjema/:behandlingsIdFraUrl/:stegFraUrl"
        });
        if (match) {
    ...
    if (behandlingsId) {
        const path = "/selvstendignaringsdrivende/skjema/:behandingsId";
        return (
            <>
                <Switch>
                    <Route path={`${path}/1`} component={Steg1}/>
                    <Route path={`${path}/2`} component={Steg8}/>
                    <Route path={`${path}/3`} component={Oppsummering}/>
...
export default connect(mapStateToProps)(withRouter(SelvstendigNaringsdrivendeSkjemaRouter) as any);
