...
const TabellView = (props: {opplysning: Opplysning; gruppeIndex: number}) => {
    ...
    const handleFjernRad = (radIndex: number, valideringsKey: string) => {
        if (behandlingsId) {
            const opplysningUpdated: Opplysning = {...props.opplysning};
            const raderUpdated: OpplysningRad[] = props.opplysning.rader.map(
                e => ({
                    ...e,
                })
            );
            ...
            validerAlleInputfelterPaOpplysning(
                opplysningUpdated,
                props.opplysning
            );
            const feilUpdated = getOppdatertListeAvFeil(
                feil,
                valideringsKey,
                radIndex
            );
    ...
    const getOppdatertListeAvFeil = (
        feil: Valideringsfeil[],
        valideringsKey: string,
        radIndex: number
    ) => {
    ...
    const validerAlleInputfelterPaOpplysning = (
        opplysningUpdated: Opplysning,
            ...
            Object.keys(rad).forEach((key: string) => {
                switch (key) {
                    case InputType.BELOP: {
                        setValideringsfeilHvisUgyldigTall(
                            InputType.BELOP,
                            rad[InputType.BELOP],
                            opplysning,
                            index
    ...
    const setValideringsfeilHvisUgyldigTall = (
        ...
        const spc: OpplysningSpc | undefined = getSpcForOpplysning(
            opplysning.type
        );
        if (spc) {
    ...
    const fjernAlleFeilForOpplysning = (
        feil: Valideringsfeil[],
        valideringsKey: string
    ) => {
    ...
    const opplysningSpc: OpplysningSpc | undefined = getSpcForOpplysning(
        props.opplysning.type
    );
    const textKey = opplysningSpc ? opplysningSpc.textKey : "";

    const innhold: JSX.Element[] = props.opplysning.rader.map(
        (vedleggRad: OpplysningRad, radIndex: number) => {
                      ...
                      return (
                          <Column xs={"12"} md={"6"} key={key}>
                              <InputEnhanced
                                  ...
                                  onBlur={() =>
                                      handleBlur(radIndex, inputType, key)
                                  }
                                  verdi={
                                      vedleggRad[inputType]
                                          ? vedleggRad[inputType]
                                          : ""
                                  }
                                  required={false}
                                  bredde={
                                      inputType === InputType.BESKRIVELSE
                                          ? "L"
                                          : "S"
                                  }
                                  faktumKey={text}
            ...
            <div className="container--noPadding container-fluid">
                ...
                {opplysningSpc &&
                    opplysningSpc.antallRader === AntallRader.FLERE && (
                        <Lenkeknapp
                            onClick={() => handleLeggTilRad()}
                            stil="add"
                            id={props.gruppeIndex + "_link"}
                        >
                            Legg til
                        </Lenkeknapp>
                    )}
