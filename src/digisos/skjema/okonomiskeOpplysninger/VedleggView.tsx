...
const VedleggView = (props: {okonomiskOpplysning: Opplysning}) => {
    ...
    const enFilLastesOpp = useSelector(
        (state: State) => state.okonomiskeOpplysninger.enFilLastesOpp
    );
    ...
    const handleAlleredeLastetOpp = (event: any) => {
        if (behandlingsId) {
            ...
            if (
                opplysningUpdated.vedleggStatus !==
                VedleggStatus.VEDLEGGALLEREDESEND
            ) {
                opplysningUpdated.vedleggStatus =
                    VedleggStatus.VEDLEGGALLEREDESEND;
            } else {
        ...
        if (behandlingsId) {
            dispatch(
                startSlettFil(
                    behandlingsId,
                    fil,
                    props.okonomiskOpplysning,
                    props.okonomiskOpplysning.type
                )
            );
        }
    };
    ...
    const renderOpplastingAvVedleggSeksjon = (opplysning: Opplysning) => {
        ...
        const tittelKey =
            opplysningSpc && opplysningSpc.textKey
                ? opplysningSpc.textKey + ".vedlegg.sporsmal.tittel"
                : "";
        ...
        const textDisabledClassName =
            opplysning.filer.length > 0 ? " checkboks--disabled" : "";
        ...
        return (
            <div>
                ...
                <Checkbox
                    ...
                    className={
                        "vedleggLastetOppCheckbox " + textDisabledClassName
                    }
                    ...
                    disabled={
                        opplysning.filer.length > 0 ||
                        opplysning.pendingLasterOppFil
                    }
                />
            </div>
        );
    ...
    return (
        <div>{renderOpplastingAvVedleggSeksjon(props.okonomiskOpplysning)}</div>
    );
};
